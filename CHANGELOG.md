...
confirmed against 2.10.0: two ways the regression gate destroyed or
misreported verified work, a structural blind spot in epic validation, a
silently ignored execution-order declaration, and a liveness signal that cried
wolf.

### Added

- **Epic mode now validates the assembled branch, not just each spec** (#10). Per-spec validation is scoped to one spec, so a defect whose acceptance criterion lives in spec N but whose production call site was wired by spec M was structurally invisible to every gate in the run — observed on a 26-story epic where 31 verifications and 4 per-spec validations all passed and a manual whole-branch validation then found a critical. After the per-spec loop, the orchestrator runs one more `validate-implementation` told explicitly to validate the ENTIRE branch diff (`main...HEAD`) against ALL of the epic's (archived) feature specs, with cross-spec integration called out; critical findings pause/block completion exactly like the per-spec passes, and anything the validation session fixes is committed before the completion strategy runs. `validate-implementation` gained a documented epic-wide mode: it gathers every spec (archive included) and feeds the compliance reviewer the full spec list instead of a single `SPEC_PATH`.
- **Story execution order can be declared, and the orchestrator honors it** (#13). Stories used to execute strictly in `### US-NNN:` document order — an "Execution order:" line written into a spec (the natural thing once validation reveals a dependency the ID numbering doesn't reflect) was a silent no-op, and renumbering isn't a workaround because sibling specs cite stories by ID. Specs can now declare `execution_order: [US-004, US-001, ...]` in frontmatter (authoritative) or an `Execution order: US-004 → US-001 → ...` line in the body; listed stories run in that order, unlisted ones follow in document order, the executor logs the effective order once per spec (with a warning for declared IDs that match no story), and the `story-quality-reviewer` validates declarations during `validate-epic`.
- **`worktree.yaml` gained a `run_prefix` contract field** (#11). A detached orchestrator inherits whatever `PATH` launched it — never an activated project shell — so `env_bootstrap` would build the worktree's environment and nothing then made the orchestrator *use* it: on an isolated project (uv/poetry/nix/hatch) the regression gate crashed at collection and reverted verified work as a false regression. `run_prefix` (e.g. `uv run`, `poetry run`) is prepended to every test/regression command the orchestrator constructs (`detect_test_command`, regression groups; user-authored TESTING_GUIDE commands are used verbatim), taking precedence over the interpreter auto-detection below. The template documents it, `execute-epic` warns when an isolated project lacks it, and the doctor accepts the new key.

### Fixed

- **A regression group that never ran is no longer reported as "Passed"** (#12). When a group timed out (or its runner was missing), the gate logged "skipping" and then reported `Passed (N test files ...)` — the computed `"skipped"` status was discarded by a wrapper and the summary line was unconditional, so 18 never-run files merged under a line that read as confirmation. The status now survives to the summary: a partial run reports `Passed (...) · NOT RUN: ...`, a fully-skipped check reports `Skipped`, and tests that never ran are no longer recorded as passes in `test-metrics.json`. The timeout stays non-fatal by design — only the reporting was wrong.
- **A pytest run that can't collect the suite no longer reverts a verified merge** (#11 hardening). pytest exits 2/3/4/5 (collection error, internal error, usage error, no tests collected) mean the gate learned *nothing* about the code — e.g. a `conftest` import crash under the wrong interpreter — and are now skipped with a loud warning instead of being read as a regression; the flake-confirm re-run applies the same discipline. Exit 1 (tests ran and failed) still reverts.
- **The heartbeat is now a real liveness signal** (#14). `heartbeat` in `.execution-health.json` was stamped only at attempt boundaries, while `execution-status` documents a 20-minute staleness alarm — so a healthy L/XL story (25-35 min sessions) false-alarmed by construction, teaching supervisors the field cries wolf. A daemon thread now refreshes it every 60s for the whole run (guarded by a lock shared with snapshot writes; it never *creates* the file, so it can't re-dirty a cleaned-up worktree), which finally makes a genuinely wedged orchestrator inside a long session detectable. The status skill's guidance is updated accordingly, including how to read runs started by older orchestrators.
- **The regression gate runs pytest in the project's own interpreter** (#11). A detached orchestrator inherits whatever `python3` launched the CLI, not an activated project shell, so for an isolated project (uv / poetry / `venv`) that interpreter lacked the project's dependencies — every cross-story regression run crashed in `conftest` and reverted a good merge as a false regression. `_regression_cmd` now resolves the pytest interpreter from the worktree: a `<cwd>/.venv` (covers uv-synced, poetry, and plain `python -m venv` projects), then `uv run` for a uv-managed project (`uv.lock`) when `uv` is on PATH, else bare `python3` (unchanged for non-isolated repos). `detect_test_command` likewise returns `uv run pytest` for uv projects instead of a bare `python3 -m pytest` that ignores the isolated `.venv`.

### Delta from the Copilot build

- All five issue fixes are host-agnostic orchestrator logic and ported verbatim; only the port's standard mechanical deltas apply (`run_copilot_session` for `run_claude_session`, `**new_group_kwargs()` for `start_new_session=True`, `.agent.md` agent filenames, bare skill names for `/kit-tools:` slash syntax, "poller" for "cron" in supervisor prose).
- The Copilot 2.10.1 additionally ships a Copilot-only feature from its own backlog — supervisor check-ins surfaced into the Copilot chat — with no Claude-side counterpart needed (this build already reports in-session via cron).
