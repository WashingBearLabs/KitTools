...
1. **Read the contract** `kit_tools/worktree.yaml` for `root`, `env_bootstrap`, `env_link`, `path_links`, `run_prefix`. If it's still missing after step 0, use empty values and the default root. Let `<key>` = the epic name (or feature name in standalone fallback).
   ...
   **⚠️ Environment reality check (`run_prefix`).** `env_bootstrap` *creates* the environment, but the detached orchestrator does not automatically *use* it — it inherits the ambient `PATH` that launched it, not an activated project shell. If the project's runner lives inside an isolated environment (uv, poetry, nix, hatch: i.e. `env_bootstrap` creates a `.venv` or equivalent) and `run_prefix` is **empty**, the orchestrator's regression checks can crash at collection and read the crash as a real failure. Recommend setting `run_prefix` (e.g. `uv run`, `poetry run`) in `kit_tools/worktree.yaml` — the orchestrator prepends it to every test/regression command it constructs. (Python projects get a best-effort fallback — the orchestrator auto-detects a worktree `.venv` or `uv.lock` — but the explicit contract is authoritative and covers every stack.)
