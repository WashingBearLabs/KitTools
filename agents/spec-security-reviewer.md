...
**readiness_score** (1–10, see FINDING_SCHEMA.md): your execution-readiness score for *this* spec, anchored to your verdict — `not-ready` → 1–4, `needs-work` → 5–7, `ready` → 8–10. Skew low when in doubt: reserve 9–10 for a spec you'd execute unsupervised; a real caveat makes it a 7, not "a 9 with a caveat." Security omissions cut hard — when in doubt about an unhandled trust boundary, score in the lower band.
